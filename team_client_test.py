...
def main():
    try:
        ...
        # Get current user info from team API
        print("\n=== Getting Current User Info ===")
        current_info = dbx_team.team_users_get_current_account()
        print(f"User ID: {current_info.root_info.root_namespace_id}")
        print(f"Display Name: {current_info.name.display_name}")
        print(f"Email: {current_info.email}")
        print(f"Team: {current_info.team.name}")
        
        # Create a user-specific client
        user_id = current_info.root_info.root_namespace_id
        dbx = dbx_team.as_user(user_id)
        
        print(f"\n=== Accessing User: {current_info.name.display_name} ===")
